...
(function($){
  ...
  function initLoad () {
    $(window).load(function() {
      ...
      // you can change the amount of time that you want to show the preloader
  ...
  function initNextUpcoming() {
    if (document.getElementsByClassName('fs-upcoming-wrapper').length > 0) {
      document.getElementsByClassName('fs-upcoming-wrapper')[0].style.marginLeft = "auto";
      document.getElementsByClassName('fs-upcoming-wrapper')[0].style.marginRight = "auto";
    }

    if (document.getElementsByClassName('fs-table-item').length > 0) {
      for (i=0; i<document.getElementsByClassName('fs-table-item').length; i++) {
        document.getElementsByClassName('fs-table-item')[i].href = "#";
      }
    }
  }
  ...
  function initReferees () {
    ...
    $('#referees').find('.event').on('click', function(e){
      $("#loader").show();
      $("#mask").show();

      $.get($(this).attr('href'), function(data) {
          var newContent = $(data);
          $('.modal-content').empty().append(newContent);
          $("#loader").fadeOut("slow");
          $("#mask").fadeOut("slow");
      });
    });

    $('#referees').find('#events-modal').on('hidden.bs.modal', function () {
      $(this).find('.modal-content')[0].innerHTML = "";
    });
  }
